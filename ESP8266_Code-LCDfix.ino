...
// LCD Code By Boyd Chaiwut for ESP8266/Arduino Core library. 
//////////////////////////////////////////////////////////
...
namespace {
const char* SSID          = "ชื่อ WiFi";   // Change this to your WiFi name
const char* PASSWORD      = "รหัส WiFi";    // Change this to your WiFi password
const char* USERNAME      = "ยูสเซอร์";     // Change this to your Duino-Coin username
const char* RIG_IDENTIFIER = "ชื่ออุปกรณ์";       // Change this if you want a custom miner name
...
bool max_micros_elapsed(unsigned long current, unsigned long max_elapsed) {
  ...
  //---------------เซ็ตจอ LCD มอนิเตอร์ โค้ดนี้เขียนสำหรับจอ 20x4 -------------------------// 

LiquidCrystal_I2C lcd(0x27, 20, 4);
void setup() {
  lcd.begin();
  lcd.backlight();
  lcd.setCursor(5, 0); // กำหนดให้ เคอร์เซอร์ อยู่ตัวอักษรตำแหน่งที่0 แถวที่ 1 เตรียมพิมพ์ข้อความ
  lcd.print("DUINO-COIN"); //พิมพ์ข้อความ "DUINO-COIN"
  lcd.setCursor(4, 1); // กำหนดให้ เคอร์เซอร์ อยู่ตัวอักษรกำแหน่งที3 แถวที่ 2 เตรียมพิมพ์ข้อความ
  lcd.print("Start Mining"); //พิมพ์ข้อความ "Start Mining"
  ...
  for (unsigned int duco_numeric_result = 0; duco_numeric_result < difficulty; duco_numeric_result++) {
    ...
    if (result == expected_hash) {
      ...
      client.print(String(duco_numeric_result)
                   ...
                   + ","
                   + String(chipID));
  //---------------ส่งข้อมูลออกจอ LCD มอนิเตอร์ โค้ดนี้เขียนสำหรับจอ 20x4 -------------------------// 
        lcd.setCursor(0, 2);//20
        lcd.print(USERNAME); 
      ...
      Serial.println(client_buffer
        ...
        lcd.setCursor(10, 2);//20
        lcd.print("No:");
        lcd.print(share_count);
        
        lcd.setCursor(0, 3);//20
        lcd.print(hashrate / 1000, 2); 
        lcd.println(".kH/s");
        
        lcd.setCursor(11, 3);//20
        lcd.print(difficulty); 
        lcd.println("dif");
        
        //lcd.setCursor(0, 3);//20
        //lcd.print(duco_numeric_result); 
        //lcd.println("sh");
        delay(500); 
